...
def filter_sites(X, Y, ages, genders, sites):
    sites_filter = np.isin(sites, ["KKI", "NI", "NYU", "OHSU", "PKU"])
    X = X[sites_filter]
    Y = Y[sites_filter]
    sites = sites[sites_filter]
    ages = ages[sites_filter]
    genders = genders[sites_filter]
    return X, Y, ages, genders, sites


def load_data(param, X, X_harmonized, Y, ages, genders, sites, filter=False):
    X_ = X_harmonized if param["harmonized"] else X
    if filter:
        X_, Y, ages, genders, sites = filter_sites(X_, Y, ages, genders, sites)
...
def model_score(param, X, X_harmonized, Y, ages, genders, sites, force):
    ...
    if os.path.exists(score_path) and not force:
        ...
        try:
            ...
            data = load_data(
                param, X, X_harmonized, Y, ages, genders, sites, filter=True
            )
            model = load_model(param, data)
